...
内核2.5版本开发的既定目标之一就是为内核创建统一的设备模型。随着新系统的拓扑结构越来越复杂，以及对电源管理等功能的需求都表明，需要一个描述系统结构的通用抽象模型。

在内核2.6版本里，提出了`设备模型`的抽象概念。直到现在，还是内核许多任务的实现架构：

* 电源管理和系统关机

    电源管理，顾名思义，就是开关电源，也就意味着系统需要关闭。这需要了解系统结构，作出是否可以操作系统开关的判断。例如，在处理完所有连接到USB适配器的设备之前，该适配器不能关闭。设备模型可以按照正确的顺序遍历系统的硬件。
...
为了更好地理解设备驱动模型的作用，让我们逐步介绍内核中设备的生命周期。 我们描述了PCI子系统如何与驱动模型交互， 如何添加和删除驱动，以及如何在系统中添加和删除设备的基本概念。 这些描述PCI内核代码的细节， 同样适用于那些使用驱动核心程序管理驱动和设备的其它子系统。
