...
/******************************************************/
 volatile int sdio_inited = 0;
int get_sdio_init_state(void)
{
  return sdio_inited;
}
int sdcard_ocr;
int sdio_init()
{
 ...
 // int rca;
  int card_info;
  int trans_state;
  volatile int sdiintmsk = 0;
  volatile int sdsta = 0;
  int clk_sel;
  int cmd08tms = 2;
  int cmd41tms = 10000;
  int cmd03tms = 100;
  ...
  if(sdio_inited>0) return 0;
  if(sdio_inited<0) return -1;
	/*sdio use dma1*/
...
//	  *(volatile unsigned int *)(SDIO_BASE + SDIPRE) = 2; // config pre_scale
  ...
  delay(1000);//loongs 2018-12
  sdiintmsk = cmd_check(cmd_index);
	  sdsta = *(volatile unsigned int *)(SDIO_BASE + SDICMDSTA);
	  resp  = *(volatile unsigned int *)(SDIO_BASE + SDIRSP0);
	  printf("[sdio]: cmd8 sta 0x%x\n",sdsta);
	  printf("[sdio]: cmd8 rsp 0x%x\n",resp);
	  printf("[sdio]: cmd8 int 0x%x\n",sdiintmsk);
	  if(sdsta == 0xff){
	    //inited = -1;
	    return -1;
	  }	  
...
while((resp & 0x80000000) == 0)                              
{
 ...
 sdcard_ocr = resp;
 sdio_inited = 1;
printf("[sdio]: cmd41[%d] inited fin\n",cmd41tms);
  resp = 0;
  cmd_index = 2;
  wait_rsp  = 1;
  longrsp   = 1;
...
printf("[sdio]: cmd3[%d] stby state\n",cmd03tms);
  ...
  check_crc = 0;
  cmd_arg   = rca;
  send_cmd(cmd_index,wait_rsp,longrsp,check_crc,cmd_arg);  // send cmd9 get card csd ,analysis it and get capacity etc.
  cmd_check(0x3f);
  ...
  check_crc = 1;
  cmd_arg   = rca;
  send_cmd(cmd_index,wait_rsp,longrsp,check_crc,cmd_arg);  // send cmd13 get card status
  cmd_check(cmd_index);
